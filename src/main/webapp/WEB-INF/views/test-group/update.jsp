...
<html>
	<head>
		<title>eSmartLabs: Update Test Group: ${testGroup.name}</title>
	</head>
	<body>
		<div id="page-wrapper">
				...
				<div class="row">
					<div class="col-lg-12">
						<h1 class="page-header">
							Update <small>Test Group: ${testGroup.name}</small>
						</h1>
					</div>
				</div>
		
				<div class="row">
					<c:url value="#" var="updateTestGroup" />
					<form:form role="form" action="${updateTestGroup}" method="POST" modelAttribute="testGroup" id="update-test-group">
					...
					<validation:CRUD validateURL="/test-group/create" requestMethod="POST"
						...
						callBackOnSuccess="testGroupUpdateSuccess" formId="update-test-group"
						noSubmit="1" />
				</div>
			</div>
		</div>
		
		<div class="modal fade" id="test-group-update-modal" tabindex="-1"
		...
		<script type="text/javascript">
			function testGroupUpdateSuccess() {
				
				var name = $("#name").val();
				$(".custom-modal-body").text(name + " updated successfully.");
				$("#test-group-update-modal").modal('show');
			}
		</script>
	</body>
</html>